...
public class CommentService {
    ...
    public CommentUpvoteResponse upvoteComment(String accessToken, CommentRequest request) {
        // Logic to handle comment likes
        Comment comment = commentRepository.findById(request.getCommentId()).orElseThrow();
        comment.setUpvotes(comment.getUpvotes() + 1);
        commentRepository.save(comment);
        return new CommentUpvoteResponse(comment.getCommentId(), comment.getUpvotes());
    }
